...
def d_w(water_vapor_pressure, temperature):
	...
	return ((1. + p_w(water_vapor_pressure) * (1. + 3.7E-4 * p_w(water_vapor_pressure))) * 
			(-2.37321E-3 + (2.23366 / T_use) - (710.792 / np.power(T_use, 2.)) + 
			(7.75141E-4 / np.power(T_use, 3.))) * p_w(water_vapor_pressure) / T_use)
...
def refraction(zenith_angle, wavelength, atmospheric_pressure, water_vapor_pressure, 
	...
	relative_gravity = (1. + 0.005302 * np.power(np.sin(np.radians(latitude)), 2.) + 
		-0.00000583 * np.power(np.sin(np.radians(2.*latitude)), 2.) - 0.000000315 * altitude)
